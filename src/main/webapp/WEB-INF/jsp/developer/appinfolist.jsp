...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form id="form1" method="post" action="${pageContext.request.contextPath}/searchApp">
											...
											<div class="btn-group">
												...
												<ul class="dropdown-menu" role="menu">
													<c:if test="${appInfo.status==2 || appInfo.status==5}">
														<li>
															<a name="upJia" appid="${appInfo.id}" class="saleSwichOpen" saleSwitch="open"  data-toggle="tooltip" data-placement="top" title="" data-original-title="恭喜您，您的审核已经通过，您可以点击上架发布您的APP">上架</a>
														</li>
													</c:if>
													<c:if test="${appInfo.status==2 || appInfo.status==4}">
														<li>
															<a name="upJia" appid="${appInfo.id}" class="saleSwichClose" saleSwitch="close"  data-toggle="tooltip" data-placement="top" title="" data-original-title="您可以点击下架来停止发布您的APP，市场将不提供APP的下载">下架</a>
														</li>
													</c:if>


													<li><a class="addVersion" href="/goAddVersion?appid=${appInfo.id}" data-toggle="tooltip" data-placement="top" title="" data-original-title="新增APP版本信息">新增版本</a>
													</li>
													...
													<c:if test="${appInfo.status==1 || appInfo.status==3}">
														<li><a  href="${pageContext.request.contextPath}/getAppInfo?id=${appInfo.id}" class="modifyAppInfo" data-toggle="tooltip" data-placement="top" title="" data-original-title="修改APP基础信息">修改</a></li>
													</c:if>

													<li><a  class="viewApp"  data-toggle="tooltip" data-placement="top" title="" data-original-title="查看APP基础信息以及全部版本信息">查看</a></li>
													<li><a href="javascript:if (confirm('是否删除？')) location.href='${pageContext.request.contextPath}/deleteAppInfo?appid=${appInfo.id}';" class="deleteApp" data-toggle="tooltip" data-placement="top" title="" data-original-title="删除APP基础信息以及全部版本信息">删除</a></li>
												</ul>
											</div>
										</td>
									</tr>
...
<script language="JavaScript">
    ...
    $(function (){
        $("a[name='upJia']").click(function () {
            ...
            if(aobj.html()=="上架")
            {
                $.get("${pageContext.request.contextPath}/upDownApp",{"appid":appid,"status":4},function(data){
                    if(data.result==1){
                        alert("已上架");
                        aobj.html("下架");
                        //修改状态
                ...
                $.get("${pageContext.request.contextPath}/upDownApp",{"appid":appid,"status":5},function(data){
                    if(data.result==1){
                        alert("已下架");
                        aobj.html("上架");
                        //修改状态
